...
from sqlalchemy import event, Integer, String, func, Table
...
class History(db.Model):
    ...
    uid = db.Column(db.String(20), nullable=False)
    hid = db.Column(db.String(20), nullable=False)
    hname = db.Column(db.String(50), nullable=False)
    bedsselected = db.Column(db.Integer, nullable=False)
    costperbed = db.Column(db.Integer, nullable=False)
    totalcost = db.Column(db.Integer, nullable=False)
    ...
    def __repr__(self) -> str:
        return f"{self.sno} - {self.uid} - {self.hid} - {self.hname} - {self.bedsselected} - {self.costperbed} - {self.totalcost} - {self.action}"
...
def drop_tables():
    with app.app_context():
        db.drop_all()
        # db.metadata.drop_all(db.engine, tables=[Beds.__table__])
...
def cost():
    if request.method == 'POST':
        uid=request.form['uid']
        c= Costs.query.filter_by(uid=uid).first()
        if c:
            return render_template('errorcost.html')
        else:
            hid = request.form['hid']
            ...
            if homeObject:
                ...
                if (int(bed.bedsAvailable) < int(bedsselected)):
                    ...
                    bed.bedsAvailable = str(
                        int(bed.bedsAvailable)-int(bedsselected))
...
@app.route('/vacate', methods=['GET', 'POST'])
def vacate():
    return render_template('vacate.html')


@app.route('/retain', methods=['GET', 'POST'])
def retain():
    if request.method == 'POST':
        uid = request.form['uid']
        hname = request.form['hname']
        password = request.form['password']
        beds=request.form['bedsselected']
        home = Home.query.filter_by(uid=uid).first()
        Cst=Costs.query.filter_by(uid=uid).first()
        hnameObject = Costs.query.filter_by(hname=hname).first()
        if Cst and hnameObject:
            if (home.password == password):
                cost = Costs.query.filter_by(uid=uid).first()
                cost=cost.query.filter_by(bedsselected=beds).first()
                if (cost.hname == hname):
                    bed = Beds.query.filter_by(hid=cost.hid).first()
                    bed.bedsAvailable = str(
                        int(bed.bedsAvailable)+int(cost.bedsselected))
                    history = History(uid=uid, hid=bed.hid, hname=hname, bedsselected=cost.bedsselected,
                                      costperbed=cost.costperbed, totalcost=cost.totalcost, action="Discharged")
                    if cost:
                    ...
                    return render_template('errorHospitalName.html')
            else:
...
if __name__ == "__main__":
    with app.app_context():
        ...
        # db.session.commit()
        # bedRows.clear()
        # Check if the data is already added before inserting
        if not Beds.query.first():
            for hospitalRow in hospitalRows:
                hospital = Hospitals(
                    hid=hospitalRow["hid"], hname=hospitalRow["hname"], haddress=hospitalRow["address"])
                db.session.merge(hospital)
